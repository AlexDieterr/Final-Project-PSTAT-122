...
lights_levels <- c("off", "on")        # A
music_levels  <- c("no", "yes")        # B
time_levels   <- c("morning", "evening")  # C
...
random_schedule <- design_grid %>%
  group_by(subject) %>%
  slice_sample(n = 8) %>%              # 8 treatment combinations per subject
